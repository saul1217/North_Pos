        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-auto bg-black/40 p-4 md:p-8" role="dialog" aria-modal="true" aria-labelledby="product-form-title">
          <form onSubmit={submitForm} className="w-full max-w-2xl border border-north-border bg-white shadow-xl">
            ...
            <div className="grid gap-4 p-5 md:grid-cols-2">
              ...
              <label className="text-sm font-medium">SKU<input required value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value, barcode: e.target.value })} className="mt-1 h-10 w-full border border-north-border px-3 font-normal" /><span className="mt-1 block text-xs font-normal text-north-muted">Identificador interno del producto.</span></label>
              ...
              <label className="text-sm font-medium">Código local Code 128<input readOnly value={form.barcode} className="mt-1 h-10 w-full border border-north-border bg-north-background px-3 font-mono text-sm font-normal" /><span className="mt-1 block text-xs font-normal text-north-muted">Se genera automáticamente con el SKU.</span></label>
