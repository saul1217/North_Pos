...
export function ChangePasswordScreen({ session, onChanged }: { session: AuthSession; onChanged: (session: AuthSession | null) => void }) {
  ...
  async function submitEmailCode(event: FormEvent) {
    ...
    catch { setError("Código incorrecto o expirado."); }
    finally { setSaving(false); }
  }
  ...
  async function submitPassword(event: FormEvent) {
    ...
    catch (reason) { setError(readError(reason, "No se pudo cambiar la contraseña.")); }
    finally { setSaving(false); }
  }
...
function readError(reason: unknown, fallback: string) {
  ...
  try {
    const payload = JSON.parse(reason.message) as { message?: string | string[] };
    if (Array.isArray(payload.message)) return payload.message.join(" ");
    if (payload.message) return payload.message;
  } catch {
    // Keep technical server responses out of the POS interface.
  }
  return fallback;
}
