...
import { confirmPasswordChange, requestEmailVerification, requestPasswordChange, verifyEmail } from "@/lib/catalog/api";
import { clearAuthSession, saveAuthSession, type AuthSession } from "@/lib/auth";
import { logoSrc } from "@/lib/brand";

type Step = "email" | "email-code" | "password" | "password-code";
...
export function ChangePasswordScreen({ session, onChanged }: { session: AuthSession; onChanged: (session: AuthSession | null) => void }) {
  ...
  async function submitPasswordRequest(event: FormEvent) {
    event.preventDefault(); setError(null); setSaving(true);
    try { await requestPasswordChange(); setMessage("Te enviamos un código para confirmar la nueva contraseña."); setStep("password-code"); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "No se pudo enviar el código."); }
    finally { setSaving(false); }
  }
  ...
  async function submitPassword(event: FormEvent) {
    event.preventDefault(); setError(null); setSaving(true);
    try { await confirmPasswordChange(passwordCode.trim(), newPassword); clearAuthSession(); onChanged(null); }
